...
getcaddr(struct con *cp) {
	...
	if (getsockname(cp->fd, sep, &len) == -1)
		return;
	error = getnameinfo(sep, SA_LEN(*sep), cp->caddr, sizeof(cp->caddr),
...
initcon(struct con *cp, int fd, struct sockaddr *sa)
{
	...
	cp->stutter = (greylist && !grey_stutter && cp->blacklists == NULL) ?
	    0 : stutter;
	error = getnameinfo(sa, len, cp->addr, sizeof(cp->addr), NULL, 0,
	    NI_NUMERICHOST);
	if (error)
...
main(int argc, char *argv[])
{
	...
	while ((ch =
		...
		case 'b':
			greylist = 0;
			errx(1, "blacklist only mode not supported");
			break;
		case 'G':
			if (sscanf(optarg, "%ld:%ld:%ld", &passtime, &greyexp,
			    &whiteexp) != 3)
				usage();
	...
	if ((pw = getpwnam("spamd")) == NULL)
		errx(1, "no such user spamd");
