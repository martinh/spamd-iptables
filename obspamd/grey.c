...
int
configure_iptables(char **addrs, int count)
{
	ipt_chainlabel		 chain = "spamd-white";
	...
	if (debug)
		fprintf(stderr, "[%d] configuring iptables\n", getpid());

	memset(&e, 0, sizeof(e));
	e.entry.target_offset = offsetof(struct rule, target);
	e.entry.next_offset = offsetof(struct rule, target) + 36; //offsetof(struct ipt_entry_target, data);

/*	e.target.u.target_size = IPT_ALIGN(sizeof(struct ipt_entry_target)); */
/*	e.target.u.target_size = offsetof(struct ipt_entry_target, data); */
	e.target.u.target_size = 36; /* WTF!? */
	strcpy(e.target.u.user.name, "ACCEPT");

	if ((h = iptc_init("nat")) == NULL) {
		syslog(LOG_ERR, "failed to initialize iptables: %s", iptc_strerror(errno));
		if (debug)
			fprintf(stderr, "[%d] failed to initialize iptables: %s\n", getpid(), iptc_strerror(errno));
		return -1;
	}

	if (!iptc_is_chain(chain, h)) {
		syslog(LOG_ERR, "%s: no such chain", chain);
		goto fail;
	}

	if (!iptc_flush_entries(chain, h)) {
		syslog(LOG_ERR, "failed to flush chain %s: %s", chain, iptc_strerror(errno));
		goto fail;
	}

	for (i = 0; i < count; i++)
		if (addrs[i] != NULL) {
			if (i > 0 && strcmp(addrs[i], addrs[i - 1]) == 0) {
				if (debug)
					fprintf(stderr, "[%d] skipping duplicate %s/32\n", getpid(), addrs[i]);
				continue;
			}

			if (debug)
				fprintf(stderr, "[%d] whitelisting %s/32\n", getpid(), addrs[i]);

			e.entry.ip.src.s_addr = inet_addr(addrs[i]);
			e.entry.ip.smsk.s_addr = inet_addr("255.255.255.255");
			if (!iptc_append_entry(chain, &e.entry, h)) {
				syslog(LOG_ERR, "failed to insert rule %d: %s", i + 1, iptc_strerror(errno));
				goto fail;
			}
		}

	strcpy(e.target.u.user.name, "RETURN");
	memset(&e.entry.ip, 0, sizeof(e.entry.ip));

	if (!iptc_append_entry(chain, &e.entry, h)) {
		syslog(LOG_ERR, "failed to insert RETURN rule: %s", iptc_strerror(errno));
		goto fail;
	}

	if (iptc_commit(h) == 0) {
		syslog(LOG_ERR, "failed to commit iptables: %s", iptc_strerror(errno));
		goto fail;
	}
...
fail:
	if (debug)
		fprintf(stderr, "[%d] configure_iptables failed: %s\n", getpid(), iptc_strerror(errno));
	iptc_free(h);
	return -1;
}
...
greyscan(char *dbname)
{
	...
	db->close(db);
	db = NULL;
	configure_iptables(whitelist, whitecount);
...
greywatcher(void)
{
	...
	setproctitle("[priv] (iptables <spamd-white> update)");
	greyscanner();
	/* NOTREACHED */
	exit(1);
}
